...
def convert_cols(cols, scale_factor, sid, invalid_data_behavior):
    ...
    """
    scaled_opens = (np.nan_to_num(cols["open"].astype(float)) * scale_factor).round()
    scaled_highs = (np.nan_to_num(cols["high"].astype(float)) * scale_factor).round()
    scaled_lows = (np.nan_to_num(cols["low"].astype(float)) * scale_factor).round()
    scaled_closes = (np.nan_to_num(cols["close"].astype(float)) * scale_factor).round()
...
class BcolzDataMinuteBundle(AbstractDataBundle):
    ...
    def write(self, data, calendar: ExchangeCalendar, start_session: pd.Timestamp, end_session: pd.Timestamp,
        ...
        with ctx as it:
            for e in it:
                # return self._write_internal(sid=e[0], df=e[1], assets=assets, calendar=calendar,
                ...
                #                             )
                write_sid(sid=e[0], df=e[1], invalid_data_behavior=invalid_data_behavior)
...
class BcolzMinuteBarReader(MinuteBarReader):
    ...
    def get_fields(self) -> list[str]:
        return ["open", "high", "low", "volume", "close", "price"]
