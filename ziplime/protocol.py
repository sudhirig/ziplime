...
from ziplime.assets.assets import Asset
        ...
        if isinstance(assets, Asset):
            ...
            return pd.Series(data={
                asset: self._is_stale_for_asset(
                    asset=asset, dt=dt, adjusted_dt=adjusted_dt, data_portal=data_portal
                )
                for asset in assets
            })

    def _is_stale_for_asset(self, asset: Asset, dt: pd.Timestamp, adjusted_dt: pd.Timestamp,
                            data_portal: DataPortal) -> bool:
        ...
        if current_volume > 0:
            ...
            last_traded_dt = \
                data_portal.get_spot_value(assets=[asset], field="last_traded", dt=adjusted_dt,
                                           data_frequency=self.data_frequency)
