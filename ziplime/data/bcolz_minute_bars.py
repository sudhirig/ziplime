...
def convert_cols(cols, scale_factor: int, sid: int, invalid_data_behavior: str):
    ...
    for col in cols.keys():
        scaled = (np.nan_to_num(cols[col]) * scale_factor).round()
        scaled_data.append((col, scaled))
    ...
    exclude_mask = np.zeros_like(scaled_data[0][1], dtype=bool)
    result = []
    for col_name, scaled_col in scaled_data:
        ...
        converted_to_uint32 = scaled_col.astype(np.uint32)

    for col_name, scaled_col in scaled_data:
        converted_to_uint32 = scaled_col.astype(np.uint32)
        converted_to_uint32[exclude_mask] = 0
        result.append(scaled_col)

    return result


class ZiplimeBcolzMinuteBarMetadata:
    """
...
class ZiplimeBcolzMinuteBarWriter:
    ...
    def __init__(
        ...
        if write_metadata:
            metadata = ZiplimeBcolzMinuteBarMetadata(
    ...
    def open(cls, rootdir: str, cols: list[ColumnSpecification], end_session: pd.Timestamp = None):
        ...
        """
        metadata = ZiplimeBcolzMinuteBarMetadata.read(rootdir)
        return ZiplimeBcolzMinuteBarWriter(
            rootdir,
            metadata.calendar,
    ...
    def _init_ctable(self, path):
        ...
        table = ctable(
            rootdir=path,
            columns=[initial_array for _ in self.COL_NAMES],
    ...
    def _write_cols(self, sid, dts, cols, invalid_data_behavior: str):
        ...
        zeroed_columns = []
        for column in cols:
            col = np.zeros(minutes_count, dtype=np.uint32)
    ...
    def truncate(self, date):
        ...
        metadata = ZiplimeBcolzMinuteBarMetadata.read(self._rootdir)
        metadata.end_session = date
        metadata.write(self._rootdir)


class ZiplimeBcolzMinuteBarReader(MinuteBarReader):
