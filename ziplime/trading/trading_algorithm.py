...
class TradingAlgorithm:
    ...
    def __init__(
            ...
            trading_signal_executor: TradingSignalExecutor,
        ...
        self.trading_signal_executor = trading_signal_executor
        self.config = config
        ...
        self._ledger = Ledger(trading_sessions=self.sim_params.sessions, capital_base=self.sim_params.capital_base,
                              data_bundle=self.data_bundle, data_frequency=self.sim_params.emission_rate)
    ...
    def init_engine(self, get_loader):
        ...
        if get_loader is not None:
            self.engine = SimplePipelineEngine(
                get_loader,
                self.data_bundle.asset_repository,
    ...
    def calculate_capital_changes(
            ...
            portfolio_value_adjustment: Decimal = Decimal(0)
    ):
    ...
    async def symbol(
        ...
        """
        Asynchronously retrieves an Asset corresponding to the specified symbol and its associated details,
        including asset type and exchange name. If no asset matches the given criteria,
        the method raises exception SymbolNotFound

        Args:
            symbol_str: The unique identifier of the asset (e.g., stock ticker).
            asset_type: The type of the asset to be retrieved. Defaults to AssetType.EQUITY.
            exchange_name: The name of the exchange where the asset is traded. Optional.

        Returns:
            Asset | None: The asset object matching the specified criteria, or None if no matching asset
            is found.
        ...
        """
        return await self.asset_service.get_asset_by_symbol(symbol=symbol_str, asset_type=asset_type,
                                                            exchange_name=exchange_name)

    async def symbols(self, *args, asset_type: AssetType = AssetType.EQUITY, exchange_name: str = None):
        """
        Fetches a list of symbols for given identifiers, filtering by symbol, asset type and exchange name

        Args:
            *args: Variable length argument list of identifiers for which symbols should be created.
            asset_type: The asset type of the symbols (default is AssetType.EQUITY).
            exchange_name: The name of the exchange where the symbols are traded (default is None).

        Returns:
            list: A list of symbol objects for the given identifiers with the specified attributes.
        """
        tasks = [self.symbol(symbol_str=identifier, asset_type=asset_type, exchange_name=exchange_name)
                 for identifier in args]
        return asyncio.gather(*tasks)

    async def sid(self, sid: int) -> Asset | None:
        """
        Retrieves an asset from the asset repository using its SID (Security Identifier).

        This asynchronous function queries the `asset_repository` within the
        `data_bundle` to fetch the asset corresponding to the given SID. Returns the
        asset if found; otherwise, returns `None`.

        Args:
            sid: The Security Identifier (SID) of the asset to retrieve.

        Returns:
            Asset: The asset corresponding to the given SID, or `None` if no
            asset is found.
        """
    ...
    async def future_symbol(self, symbol: str, exchange_name: str = None) -> FuturesContract | None:
        """
        Fetches a futures contract corresponding to the given symbol and exchange name.

        This function retrieves a futures contract object for the specified
        symbol and exchange. If the exchange name is not provided, it defaults to the
        exchange name specified in simulation parameters. The fetched object represents a
        contract for trading in futures markets and provides details specific to the symbol
        and exchange.

        Args:
            symbol (str): The symbol of the futures contract to retrieve.
            exchange_name (str, optional): The name of the exchange to fetch the
                futures contract from. If not provided, the default exchange name
                from simulation parameters is used.

        Returns:
            FuturesContract | None: The retrieved futures contract if available;
            otherwise, None.
        """
    ...
    def _calculate_order_value_amount(self, asset: AssetModel, value: float):
    ...
    def _can_order_asset(self, asset: AssetModel):
    ...
    async def order(self, asset: AssetModel, amount: float, style: ExecutionStyle) -> Order | None:
        ...
        trading_signal = self.trading_signal_executor.create_order_execute_trading_signal(order=order)

        # this is actually order execution
        await self.submit_trading_signal(trading_signal=trading_signal)
        # TODO: verify this
        # process should be  -> trading signal submitted, order submitted from trading signal executor, callback called
        submitted_order = await self.exchange.submit_order(order=order)
        return submitted_order

    def _calculate_order(
            self, asset: AssetModel, amount: float,
        ...
        #     asset=asset, limit_price=limit_price, stop_price=stop_price, style=style
        # )
        return amount, style
    ...
    @api_method
    @disallowed_in_before_trading_start(OrderInBeforeTradingStart())
    async def order_value(self, asset: AssetModel, value: float, limit_price: float | None = None,
    ...
    async def order_percent(
            self, asset: AssetModel, percent: float, style: ExecutionStyle
    ):
    ...
    def _calculate_order_percent_amount(self, asset: AssetModel, percent: float):
    ...
    async def order_target_value(
            self, asset: AssetModel, target: float, style: ExecutionStyle
    ):
    ...
    async def order_target_percent(
            self, asset: AssetModel, target: float,
            style: ExecutionStyle
    ):
        ...
        async with (AsyncExitStack() as stack):
            ...
            for dt, action in self.clock:
                if action == SimulationEvent.BAR:
                    ...
                    # await self.data_bundle.asset_repository.retrieve_all(
    ...
    async def submit_trading_signal(self, trading_signal: TradingSignal):
        self.trading_signal

    async def on_submitted_order(self, order: Order):
