...
class SqliteAssetRepository(AssetRepository):
    ...
    async def save_equity_symbol_mappings(self, equity_symbol_mappings: list[EquitySymbolMapping]) -> None:
        async with self.session_maker() as session:
            session.add_all(equity_symbol_mappings)
            await session.commit()
    ...
    async def get_asset_by_symbol(self, symbol: str) -> Asset | None:
        async with self.session_maker() as session:
            ...
            if equity_mapping is None:
                return None
            q_asset = select(Asset).where(Asset.sid == equity_mapping.sid)
            asset = (await session.execute(q_asset)).scalar_one_or_none()
            return asset

    def migrate(self) -> None:
        alembic_dir_path = Path(pathlib.Path(__file__).parent.parent.parent, "alembic")
        db_path = self.get_database_path(bundle_name=self._bundle_name, bundle_version=self._bundle_version)
        ...
        alembic_cfg.set_main_option(
            "sqlalchemy.url",
            self.get_database_url_sync(bundle_name=self._bundle_name, bundle_version=self._bundle_version))
    ...
    async def retrieve_all(self, sids: list[int], default_none: bool = False):
        ...
        async with self.session_maker() as session:
            q = select(Asset).where(Asset.sid.in_(sids))
