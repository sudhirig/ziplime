...
class BundleService:
    ...
    async def ingest_bundle(self, name: str,
        ...
        for idx, asset in enumerate(assets.iter_rows(named=True)):
            ...
            # data.filter( pl.col("symbol")==asset["symbol"] )
            data = data.with_columns(sid=pl.when(pl.col("symbol")==asset["symbol"]).then(asset_db.sid).otherwise(data["sid"]))
        ...
        # assets_writer = AssetDBWriter(assets_db_path)
    ...
    async def clean(self, name, before=None, after=None, keep_last=None):
