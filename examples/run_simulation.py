...
if __name__ == "__main__":
    ...
    algorithm_file = Path("./test_algo.py").absolute()
    sim_params = SimulationParameters(
        start_date=datetime.datetime(year=2024, month=10, day=2, tzinfo=calendar.tz),
        end_date=datetime.datetime(year=2024, month=10, day=10, tzinfo=calendar.tz),
        trading_calendar=calendar,
        capital_base=100000.0,
        emission_rate=datetime.timedelta(minutes=1),
        max_shares=10000,
        exchange=exchange_class,
        bundle_name="limex_us_polars_minute_3",
    )
